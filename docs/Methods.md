...
1. 選擇：
由根節點開始，根據一定的選擇算法，找到一個葉節點（終端節點），傳統的 MCTS 會使用 UCT 作為選擇的依據，選擇 UCT 值最大的子節點往下搜尋，直到葉節點（終端節點，此節點尚無價值數值），它的公式如下
...
* 為甚麼輸入是十八層 planes 而不是 AlphaGo Zero 的十七層？
...
* 為甚麼沒有實做 AlphaGo 的快速走子（Fast Policy）網路？

根據我對 AlphaGo 歷史的理解，快速走子網路的作用跟前面 "審局函數" 所述的快速走子作用是相同的，因為 AlphaGo 早期的版本沒有價值網路，需要通過快速走子估計當前盤面分數，而現代的神經網路非常大，準確度非常高，快速走子已經沒有必要在使用。


* 為甚麼 AlphaGo 的價值網路和策略網路是分開的？

我猜這主要是歷史原因，AlphaGo 是先實做策略網路，後來才實做價值網路，最後才合併。事實上合併後的推論速度較快，網路的準確率較高，所以分開並沒有好處。


* 為甚麼 AlphaGo Zero 在優勢時會持續退讓？

一般人會認為這和蒙地卡羅樹搜索有關，但並不完全是，事實上和強化學習的系統比較有關系，因為在自我對戰的過程中，並沒有強迫它下可以贏較多的位置，導致產生的自戰棋譜在優勢時品質降低。如果你用高強度的人類棋譜訓練，你會發現蒙地卡羅樹搜和神經網路的組合並不會有明顯的退讓行為。


* 是否建議實做強化學習？

如果是圍棋的話，並不建議，因為強化學習需要大量的計算，Python 甚至 C++ 都需要消耗大量時間。如果非得實做強化學習，建議實做五子棋或黑白棋等狀態空間較小的遊戲。
